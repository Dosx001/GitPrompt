...
int main() {
    ...
    if (fgets(buffer, max_buffer, popen("git branch --show-current 2> /dev/null", "r"))) {
        ...
        cout << "\033[90m\n" << fgets(buffer, max_buffer, popen("git log -1 --pretty=format:'%s' 2> /dev/null", "r")) << '\n';
        bool check = false;
        ...
        if (check) cout << '\n';
        fgets(buffer, max_buffer, popen("basename $PWD", "r"));
        cout << "\033[32;44m";
        print(buffer, 0);
        cout << "\033[34;41m\033[30;41m";
        fgets(buffer, max_buffer, popen("git branch --show-current 2> /dev/null", "r"));
        print(buffer, 0);
        cout << "\033[00m\033[31m\033[00m";
        pclose(status);
    }
    else {
        fgets(buffer, max_buffer, popen("basename $PWD", "r"));
